...
# Verify dataset exists before running evaluator
echo "[run_evaluator] Verifying dataset location..." >&2
DATASET_DIR="$PROBLEM_DIR/data/sift1M"
if [[ -d "$DATASET_DIR" ]]; then
  echo "[run_evaluator] Dataset directory exists: $DATASET_DIR" >&2
  echo "[run_evaluator] Files in dataset directory:" >&2
  ls -la "$DATASET_DIR" >&2 || echo "[run_evaluator] Cannot list directory" >&2
else
  echo "[run_evaluator] WARNING: Dataset directory does not exist: $DATASET_DIR" >&2
  echo "[run_evaluator] Current directory: $(pwd)" >&2
  echo "[run_evaluator] PROBLEM_DIR: $PROBLEM_DIR" >&2
  echo "[run_evaluator] Contents of PROBLEM_DIR:" >&2
  ls -la "$PROBLEM_DIR" >&2 || echo "[run_evaluator] Cannot list PROBLEM_DIR" >&2
fi
