...
# evaluate.sh wrapper for llm_sql_large
PROBLEM_DIR=$(pwd)
echo "[evaluate] Running LLM_SQL_LARGE evaluation..." >&2
echo "[evaluate] Current dir: $(pwd)" >&2

# Try to run the evaluator, capture output
if ./run_evaluator.sh 2>&1 | tee /tmp/eval_output_llm_sql_large.log; then
    echo "[evaluate] run_evaluator succeeded" >&2
else
    echo "[evaluate] ERROR: run_evaluator.sh failed!" >&2
    echo "[evaluate] Output:" >&2
    cat /tmp/eval_output_llm_sql_large.log >&2
    echo "-10000"
    exit 0
fi
