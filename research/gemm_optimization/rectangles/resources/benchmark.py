...
def summarize_speedup(answer_matmul, baseline_matmul=torch.matmul, print_output=False):
    ...
    _warmup_gpu(10)
    # Tall/skinny and wide/short rectangles only
    shapes = []
    tall_M = [1024, 2048, 4096, 8192]
    skinny_N = [64, 128, 192, 256]
    K_balanced = [512, 1024, 2048]
    from itertools import product
    for M, N, K in product(tall_M, skinny_N, K_balanced):
        shapes.append((M, N, K))
    wide_N = [1024, 2048, 4096, 8192]
    short_M = [64, 128, 192, 256]
    for M, N, K in product(short_M, wide_N, K_balanced):
        shapes.append((M, N, K))
    rows = []
    for (M, N, K) in shapes:
