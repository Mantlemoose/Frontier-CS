...
def summarize_speedup(answer_matmul, baseline_matmul=torch.matmul, print_output=False):
    ...
    _warmup_gpu(10)
    # Near-tile boundary cases only
    def around(x):
        return [x-1, x, x+1, x+7]
    tile_m, tile_n, tile_k = 128, 128, 64
    tiles_M = around(tile_m) + [2*tile_m-1, 3*tile_m+1, 5*tile_m-7]
    tiles_N = around(tile_n) + [2*tile_n+1, 3*tile_n-1, 5*tile_n+3]
    tiles_K = around(tile_k) + [2*tile_k+1, 3*tile_k-1, 5*tile_k+5]
    shapes = []
    from itertools import product
    for M, N, K in product(tiles_M, tiles_N, tiles_K):
        if M > 0 and N > 0 and K > 0 and M <= 8192 and N <= 8192 and K <= 8192:
            shapes.append((M, N, K))
    rows = []
    for (M, N, K) in shapes:
