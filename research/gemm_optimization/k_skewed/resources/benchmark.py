...
def summarize_speedup(answer_matmul, baseline_matmul=torch.matmul, print_output=False):
    ...
    _warmup_gpu(10)
    # K-skewed: very small and huge K only
    shapes = []
    small_K = [32, 48, 64, 96, 128]
    huge_K  = [3072, 4096, 6144, 8192]
    base_MN = [(1024, 1024), (2048, 512), (512, 2048), (4096, 256), (256, 4096)]
    from itertools import product
    for (M, N) in base_MN:
        for K in small_K + huge_K:
            shapes.append((M, N, K))
    rows = []
    for (M, N, K) in shapes:
